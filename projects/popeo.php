<div>
	<?php
	echo projectInfo(
		"<p>J'ai travaillé à l'intégration de la maquette (aussi bien en frontend qu'en backend) et en collaboration avec un développeur <i>Senior</i> qui s'occupait de, globalement, tout le reste.<p>",
		null,
		"down"
	);
	?>
</div>
<div class="template cols-2">
	<?php 
	echo projectCard(
		"img/projets/sdw/popeo/accueil.png"
	);
	echo projectCard(
		"img/projets/sdw/popeo/accueil-comment-ca-fonctionne.png"
	);
	?>
</div>
<div class="template cols-3">
	<?php
	echo projectCard(
		"img/projets/sdw/popeo/accueil-contact.png"
	);
	echo projectCard(
		"img/projets/sdw/popeo/accueil-suite-3.png"
	);
	echo projectCard(
		"img/projets/sdw/popeo/connexion.png"
	);
	?>
</div>
<div style="margin-top:50px;">
	<?php
	echo projectInfo(
		"Attention à partir d'ici il y a beaucoup de formulaires.",
		null,
		"down"
	);
	?>
</div>
<div class="template cols-3">
	<?php
	echo projectCard(
		"img/projets/sdw/popeo/compte.png"
	);
	echo projectCard(
		"img/projets/sdw/popeo/espace-pub.png"
	);
	echo projectCard(
		"img/projets/sdw/popeo/paiements.png"
	);
	echo projectCard(
		"img/projets/sdw/popeo/tableau-de-bord.png"
	);
	?>
</div>